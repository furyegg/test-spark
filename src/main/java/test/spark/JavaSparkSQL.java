...
//		test(ctx, sqlContext);
	...
	private static void jdbc(JavaSparkContext ctx, SQLContext sqlContext) {
		Map<String, String> options = Maps.newHashMap();
		options.put("driver", "net.sourceforge.jtds.jdbc.Driver");
		options.put("url", "jdbc:jtds:sqlserver://172.20.20.224;instance=SQL2008R2;databaseName=kyle;user=sa;password=password");
		options.put("dbtable", "people");
		DataFrame peopleDF = sqlContext.read().format("jdbc").options(options).load();
		peopleDF.printSchema();

		System.out.println("rows: " + peopleDF.collectAsList().size());
		...
		options2.put("driver", "net.sourceforge.jtds.jdbc.Driver");
		options2.put("url", "jdbc:jtds:sqlserver://172.20.20.224;instance=SQL2008R2;databaseName=kyle;user=sa;password=password");
		options2.put("dbtable", "location");
		DataFrame locDF = sqlContext.read().format("jdbc").options(options2).load();
		locDF.printSchema();
		locDF.registerTempTable("location");

		DataFrame locations = sqlContext.sql("SELECT * FROM location");

		DataFrame city = teenagers.join(locations, "name");
		city.toJavaRDD().map(new Function<Row, String>() {
			@Override
			public String call(Row row) throws Exception {
				return String.format("%s lives in %s", row.getAs("name"), row.getAs("city"));
			}
	...
	private static void test(JavaSparkContext ctx, SQLContext sqlContext) {
